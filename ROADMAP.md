...
- [ ] **P4-1** GIF 导出改为逐帧渲染+即时写入流，避免 360 帧 PIL Image 全部驻留内存
- [ ] **P4-2** GIF 导出使用 blitting 或预渲染优化，减少逐帧 `ax.clear()` + 全量重绘开销
- [ ] **P4-3** Excel 导出改为批量写入（`ws.append` 行写入替代逐单元格）

### Phase 5 — 代码架构（v0.6）

目标：拆分大类，改善可维护性。

- [ ] **P5-1** 拆分 `CamSimulator` 为独立模块：
  ```
  ui/
  ...
  └── app.py          # 主窗口组装与启动
  ```
- [ ] **P5-2** 提取参数数据模型类（`ParameterModel`），分离计算状态与 GUI 状态
- [ ] **P5-3** `_build_gui` 拆分为 `_build_sidebar`、`_build_toolbar`、`_build_figure` 等子方法

### Phase 6 — 功能增强（v0.7）

目标：扩展仿真功能，提升工程实用性。

- [ ] **P6-1** 滚子从动件支持：增加滚子半径参数，计算实际廓形（等距曲线）与理论廓形
- [ ] **P6-2** 压力角曲线图：新增压力角随转角变化的独立图表
- [ ] **P6-3** 凸轮轮廓曲率半径计算与显示，标注最小曲率半径位置
- [ ] **P6-4** 参数预设系统：保存/加载常用凸轮配置（JSON 文件）
- [ ] **P6-5** 压力角阈值可配置（侧边栏输入框或下拉选择）
- [ ] **P6-6** `N_POINTS` 可配置化，支持更高或更低的离散分辨率

### Phase 7 — 体验升级（v0.8）

目标：打磨交互细节，提升视觉与操作体验。

- [ ] **P7-1** 深色模式支持（tkinter 主题 + matplotlib 样式切换）
- [ ] **P7-2** 动画帧进度条：显示当前帧/总帧数，支持拖动跳转
- [ ] **P7-3** 窗口缩放自适应：监听 `Configure` 事件动态调整 Figure 尺寸
- [ ] **P7-4** 参数输入实时校验：输入时即时提示范围错误
- [ ] **P7-5** 导出格式扩展：支持 SVG 矢量图、CSV 数据表导出
- [ ] **P7-6** i18n 细节修复：运动规律后缀统一、Logo/进度标签国际化

### Phase 8 — 工程化与发布（v1.0）

目标：达到可发布质量，支持便捷安装与分发。

- [ ] **P8-1** 添加 `pyproject.toml`，支持 `pip install camforge`
- [ ] **P8-2** 添加版本号（`__version__`），窗口标题显示版本
- [ ] **P8-3** GitHub Actions CI：自动运行测试、代码风格检查（ruff / black）
- [ ] **P8-4** 代码覆盖率目标：`cam_mechanics.py` ≥ 90%，整体 ≥ 70%
- [ ] **P8-5** 添加类型注解，配置 mypy/pyright 静态检查
- [ ] **P8-6** `requirements.txt` 添加上界约束，防止 numpy 2.x 等破坏性升级
- [ ] **P8-7** 变更日志（CHANGELOG.md）维护
