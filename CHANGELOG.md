...
- Architecture restructuring (SimulationResult dataclass, SimulationPipeline)
- Flat-faced follower support
- Inverse design capabilities
- Improved test coverage and CI/CD
- Performance optimizations

Please visit the new repository for the latest updates and releases.
...
### Fixed
- Reverted dropdown button style to v0.6.11 version (restored original appearance)
- Dropdown list now matches button styling when expanded
...
### Changed
- GitHub button now displays GitHub icon instead of text
- Added github.png icon file to project
...
### Fixed
- Status bar now shows success message immediately after non-GIF exports
- GIF export completion appends to existing saved files list

## [0.6.6] - 2026-04-19

### Fixed
- GIF export progress bar now works correctly with CTkProgressBar
- Status bar now shows success message after export (non-GIF exports)
- GIF export status message shown in callback after completion

## [0.6.5] - 2026-04-19

### Changed
- Removed info panel border in animation and GIF export
- Replaced application icon with new design
- Migrated GIF export progress dialog to CustomTkinter
...
### Changed
- Download options layout changed to 4 rows × 2 columns for better readability

## [0.6.1] - 2026-04-19

### Fixed
- Status bar: use `StringVar.set()` instead of `configure(text=)` for proper updates
- Download options: moved from toolbar to sidebar as independent card
- Keyboard shortcuts: support `CTkEntry` widget detection to avoid accidental triggers
- GIF animation export: restore full implementation with background thread

### Changed
- Card title changed from "语言" to "常用设置"
- Toolbar simplified: removed download checkboxes, only buttons and sliders remain
- Added i18n keys: `sidebar.group.common`, `sidebar.group.download`
...
### Changed
- Move quick preset dropdown from toolbar to sidebar (below theme selector)
- Merge "Clear Params" and "Clear Plots" buttons into single "Clear" button
- Simplify quick preset selection logic using index instead of string matching
